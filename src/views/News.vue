<template>
  <div class="news">
    <a id="twitterBird" href="https://twitter.com/OOT_out_of_tune">
        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 400 400" style="enable-background:new 0 0 400 400;" xml:space="preserve">
            <g id="_x31_0_x2013_20_x25__Black_Tint">
                <rect class="st0" width="400" height="400"/>
            </g>
            <g id="Logo__x2014__FIXED">
                <g>
                    <path class="st1" d="M153.6,301.6c94.3,0,145.9-78.2,145.9-145.9c0-2.2,0-4.4-0.1-6.6c10-7.2,18.7-16.3,25.6-26.6
                        c-9.2,4.1-19.1,6.8-29.5,8.1c10.6-6.3,18.7-16.4,22.6-28.4c-9.9,5.9-20.9,10.1-32.6,12.4c-9.4-10-22.7-16.2-37.4-16.2
                        c-28.3,0-51.3,23-51.3,51.3c0,4,0.5,7.9,1.3,11.7c-42.6-2.1-80.4-22.6-105.7-53.6c-4.4,7.6-6.9,16.4-6.9,25.8
                        c0,17.8,9.1,33.5,22.8,42.7c-8.4-0.3-16.3-2.6-23.2-6.4c0,0.2,0,0.4,0,0.7c0,24.8,17.7,45.6,41.1,50.3c-4.3,1.2-8.8,1.8-13.5,1.8
                        c-3.3,0-6.5-0.3-9.6-0.9c6.5,20.4,25.5,35.2,47.9,35.6c-17.6,13.8-39.7,22-63.7,22c-4.1,0-8.2-0.2-12.2-0.7
                        C97.7,293.1,124.7,301.6,153.6,301.6"/>
                </g>
            </g>
        </svg>
    </a>
        <div id="line">
        </div>
    <a id="blogNetidee" href="https://www.netidee.at/out-tune/">
        <img src="../assets/netideeLogo.jpg" alt="Netidee Blog">
    </a>
  </div>
</template>

<script>
export default {
  name: 'home',
  components: {
    
  }
}
</script>

<style scoped>
    /* @keyframes example {
    from {fill:#FFFFFF;}
    to {fill: #1DA1F2;}
    } */
    .st0{
        fill:rgb(30,30,30);  

    }
    .st1{
        fill:#FFFFFF;    
        transition: fill 0.2s ease-in-out;
        -moz-transition: fill 0.2s ease-in-out;
        -webkit-transition: fill 0.2s ease-in-out;
        -o-transition: fill 0.2s ease-in-out;
 
    }
    .st1:hover{
        /* animation-name: example;
        animation-duration: 0.5s; */
        fill: #1DA1F2;

    }
    #line{
        background: #da6a1d;
        /* margin-top:10vw; */
        height: 100%;
        /* border-radius: 25px; */
    }
    #blogNetidee{
        /* margin: auto auto auto auto; */
        width: 100%;
        height: 100%;

    }
    #blogNetidee img:hover{
        width: 90%;
        -webkit-box-shadow: 0px 0px 15px -5px rgba(0,0,0,0.75);
        -moz-box-shadow: 0px 0px 15px -5px rgba(0,0,0,0.75);
        box-shadow: 4px 4px 15px -5px rgba(0,0,0,0.75);
    }
    #blogNetidee img{
        margin-left: auto;
        margin-right: auto;
        width: 85%;
        height: auto;
        position: relative;
        top: 50%;
        -webkit-transform: translateY(-50%);
        -ms-transform: translateY(-50%);
        transform: translateY(-50%);
        transition: width 0.1s ease-in-out;

    }
    .news{
        background-color: rgb(255,255,255);
        display: inline-grid;
        grid-template-columns: 1fr 20px 1fr;
        width: 100%;
        height: 100vh;
        position: absolute;
    }
    #twitterBird{
        background-color:rgb(30,30,30); 
    }

    #twitterBird svg{
        position: relative;
        top: 50%;
        -webkit-transform: translateY(-50%);
        -ms-transform: translateY(-50%);
        transform: translateY(-50%);
    }
    @media only screen and (max-width: 550px) {
        .news{
            grid-template-columns: 1fr;
            grid-template-rows: 1fr 20px 1fr;
            position: relative;
            height: 100%;
        }
    }
    @media only screen and (max-height: 400px) {
        .news{
            position: relative;
            height: 100%;
        }
    }

</style>
